...
- Показывать уровень риска сохраненного кейса.
- Показывать приоритет и краткое резюме по приоритету.
- Показывать краткое объяснение результата анализа.
- Показывать план действий из сохраненного анализа.
...
- Показывать `Не найдено` для отсутствующих фактов, а не ломать экран.
...
- Сохраненный кейс показывает уровень риска.
- Сохраненный кейс показывает приоритет и краткое резюме.
- Сохраненный кейс показывает краткое объяснение анализа.
- Сохраненный кейс показывает план действий из 3-5 коротких шагов.
...
- Если данных недостаточно, экран показывает осторожный fallback и `Не найдено`.
...
1. Создать новый кейс, открыть `/history`, затем открыть detail view сохраненного кейса.
   - Ожидается отображение уровня риска, приоритета, краткого объяснения и плана действий.

2. Открыть high risk кейс из истории.
   - Ожидается высокий уровень риска, высокий или критический приоритет, осторожное объяснение и практичный план действий.
...
4. Открыть low risk кейс из истории.
   - Ожидается спокойный уровень риска, низкий приоритет и короткий план проверки фактов.
