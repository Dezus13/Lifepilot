...
- fallback `Nicht gefunden` для отсутствующих значений в старых карточках истории.
...
- fallback `Nicht gefunden` для отдельных фактов, которые не найдены даже после пересчета.
